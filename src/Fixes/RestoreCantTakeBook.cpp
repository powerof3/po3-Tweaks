...
namespace Fixes::RestoreCantTakeBook
{
	namespace BlockButton
	{
		...
		void Install()
		{
			REL::Relocation<std::uintptr_t> target{ RELOCATION_ID(50126, 51057), OFFSET_VERSIONED(0x634, 0x636, 0x646, 0x64A) };
	...
	void Install()
	{
		...
		REX::INFO("\t\tInstalled 'Can't Be Taken' book flag fix"sv);
	}
}
